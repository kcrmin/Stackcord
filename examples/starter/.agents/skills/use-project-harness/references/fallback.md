...
8. Before protected product meaning becomes canonical, verify the exact commit and fingerprint through `stackcord governance check --json`. A contributor may prepare a proposal and PR, but Git user.name or user.email never proves product authority. If review state is unavailable, do not integrate or release.

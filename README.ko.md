...
| --- | --- |
| 긴 대화 뒤 중요한 요구사항을 다시 질문함 | 답변을 제품 요약·정책·scenario·결정·미해결 질문으로 계속 저장합니다. |
| 프론트와 백엔드가 서로 다른 가정을 구현함 | 구현 전에 공유 contract와 실패 동작을 정하고 각 작업을 연결합니다. |
| 여러 사람이 다른 파일에서 같은 정책이나 DB를 동시에 변경함 | path뿐 아니라 정책·contract·DB entity·UI flow·migration 의미 충돌을 미리 찾습니다. |
| submodule의 최신 코드와 root가 가리키는 commit이 다름 | child HEAD와 root pointer를 따로 검사하고 검토된 commit만 통합합니다. |
| clone하거나 AI context가 압축되면 현재 상태를 잃음 | 저장소와 fingerprint를 다시 읽어 confirmed·stale·unknown 상태를 복구합니다. |
| 사용자와 개발자가 서로 다른 release를 테스트함 | 같은 root·UI·frontend·backend commit 조합을 하나의 RC로 고정합니다. |
...
기본 mode는 일반 팀에 필요한 Git identity, TDD, 충돌·통합·migration/rollback, 동일 RC 검증을 제공합니다. `strict-release`는 조직이 선택할 때만 SBOM·provenance·signature와 강한 승인 정책을 추가합니다.

자세한 내용: [시작](./docs/getting-started/ko.md) · [UI workspace](./docs/guides/ui-workspace-ko.md) · [Submodule](./docs/guides/submodules-ko.md) · [작업 관리](./docs/guides/task-management-ko.md) · [DBML](./docs/guides/dbdiagram-ko.md) · [Release](./docs/guides/release-ko.md) · [문제 해결](./docs/guides/troubleshooting-ko.md)

...
```text
이 GitHub 저장소의 Stackcord Plugin을 설치하고, 현재 프로젝트를 시작할 준비를 해줘.
```
...
| `contracts/` | 서비스 규칙과 component 사이의 비즈니스·동작·interface·data 규약 |
| `.harness/workspaces.yaml` | root·UI·frontend·backend 저장소 관계 |
| `.harness/work/` | 선택한 task 원본, checklist, 작업 범위와 선점 상태 |
| `.harness/governance.yaml` | 제품 책임자와 보호할 제품 의미 |
| `.harness/local/` | Git에 올리지 않고 언제든 재생성할 수 있는 로컬 관찰·context cache |
| `.agents/skills/use-project-harness/` | Plugin 없이 프로젝트를 이어가기 위한 repo-local Skill |

사용자에게 보이는 Skill은 시작·이어가기·작업 계획·협업 조정·복구와 release의 다섯 역할로 통합되어 있습니다. 이름을 외울 필요는 없습니다. 기본 mode는 일반 팀 협업에 필요한 검증만 제공하며, `strict-release`는 선택한 조직에만 SBOM·provenance·signature 같은 강한 공급망 검증을 추가합니다.

...
7. Move from discovery draft to project setup only when existing product files already establish the boundary, or the first high-impact scope answer makes the target root clear. Then use `orchestrator project init` for a new root or `orchestrator project adopt` for an existing repository, review the plan before apply, preserve user files, and rerun combined status. Initialize collaboration Git early, but add a child repository or submodule only when its ownership or lifecycle is genuinely independent.

...
1. Run `orchestrator status --json`, then `orchestrator change plan --json`. Resolve affected roles, journeys, service policies, failure scenarios, quality targets, UI states, the current exact UI baseline, contracts, DB entities, migrations, workspaces, dependencies, and acceptance evidence.

...
1. Run `stackcord status --json`, then `stackcord change plan --json`. Resolve affected roles, journeys, service policies, failure scenarios, quality targets, UI states, the current exact UI baseline, contracts, DB entities, migrations, workspaces, dependencies, and acceptance evidence.
2. Ask one material question only when its answer changes product meaning. When choices help, present 2–3 exclusive options labeled A/B/C, put the recommended option first and mark it recommended, then accept either a letter or free-form input.

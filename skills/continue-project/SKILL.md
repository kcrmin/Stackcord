...
2. Re-read actual branch, dirty state, upstream, ahead/behind/diverged state, worktrees, workspace commits, root submodule pointers, approved product meaning, service contracts, external UI authority, the exact UI baseline and source fingerprints, dependent frontend work, evidence, and selected live task provider.
...
6. When state is coherent, run `stackcord work next --json`. Exclude work blocked by unresolved policy, scenario, contract, migration, UI, dependency, ownership, evidence, or pointer conflicts.

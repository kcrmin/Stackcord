...
병렬 작업 전에 여러 변경이 의존하는 interface·contract·DBML·실패 동작을 정의합니다. Coordination이 필요하면 공유 의미 범위를 선점하고 일반적인 feature branch 또는 격리 worktree를 만들며 실패 test를 작성한 뒤 최소 동작을 구현하고 자주 통합합니다. 작은 개인 변경에는 issue나 선점을 강제하지 않습니다. 구현 중 실제 제품 결정이 드러나면 제품 checkpoint도 수정합니다.

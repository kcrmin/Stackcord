...
Root orchestration 저장소가 contracts와 조정 상태를 소유하므로 반드시 함께 clone해야 합니다. `git submodule update --init --recursive`로 root가 가리키는 commit을 받고 `update --remote`를 통합 정책으로 사용하지 않습니다.

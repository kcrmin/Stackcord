...
Before changing service purpose, policy, business rules, contracts, or governance, run ` + "`stackcord governance check --json`" + `. Change product authorities by planning ` + "`stackcord governance authority add|remove --json`" + ` first, reviewing the before/after list and policy fingerprint, and applying only with that exact ` + "`--expected-policy`" + ` value. Repeat ` + "`--subject`" + ` to group multiple authorities into one atomic plan. The write remains a proposal and must complete the configured provider review before integration or release. If governance is enabled and the selected Git provider does not identify the current account as a product authority, keep the protected change as a proposal, prepare its tests and implementation normally, and use the chosen issue or PR workflow to request review. Git user.name and user.email are display metadata, not authority. Never mark a proposal approved from cached review data; integration and release require a fresh exact-commit approval.
...
1. Treat the natural-language request as the entry point. Read ` + "`AGENTS.md`" + `, ` + "`.harness/entry.md`" + `, the manifest, workspaces, profile, selected task source, and ` + "`.harness/git-conventions.yaml`" + ` when present; do not ask the user to operate internal files.
...
6. A small private local edit needs no ticket or reservation. Before shared or risky work, define the service meaning, behavioral boundary, first failing test, semantic scope, owner, dependencies, and merge order; then synchronize the selected task source and Git work reservation. Apply the committed Git convention before creating any branch, commit, pull request, or issue; external writes still require the user's request.
...
5. Before creating a branch, commit, pull request, or issue, read ` + "`.harness/git-conventions.yaml`" + ` when present; preserve existing contribution and provider templates and ask when they conflict.
6. Never hide pull, rebase, stash, reset, clean, force-push, external write, install, or release actions.
7. If context was compacted or appears forgotten, run a full context audit before mutation.
`

...
8. Before protected product meaning becomes canonical, verify the exact commit and fingerprint through ` + "`orchestrator governance check --json`" + `. An ordinary contributor may prepare a proposal and PR, but Git user.name or user.email never proves product authority. If the selected review provider is unavailable, report approval as unknown and do not integrate or release.

...
2. Treat the initial product request as the first material answer. Persist its normalized meaning before the next question, then checkpoint every later answer batch; store no raw conversation or tone. Group independent routine questions by section with recommended options first and free-form input. Recommendations remain proposals until submitted; sensitive choices require explicit answers. Summarize accepted decisions and show current/completed/remaining sections and an approximate question range using [discovery progress](discovery.md).
...
9. Keep work management proportional. A small private local edit needs no ticket or reservation. For shared, long-lived, cross-workspace, or semantically risky work, choose exactly one live task source, save the executable checklist, refresh external state when applicable, then acquire the Git work reservation before creating a branch/worktree. Read `.harness/git-conventions.yaml` when present before naming a branch, commit, pull request, or issue.

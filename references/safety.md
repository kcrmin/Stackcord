...
- Git `user.name` and `user.email` are not authentication. For protected service purpose, policy, business rules, contracts, or governance, accept only a configured product authority proven by a fresh exact-commit observation from the selected Git review provider. A non-authority can propose and implement but cannot approve, integrate, or release the protected change.
